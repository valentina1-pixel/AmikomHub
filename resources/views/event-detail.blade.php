...
<main class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
    {{-- Sisi Kiri: Gambar dan Penyelenggara --}}
    <div class="lg:col-span-1">
        <div class="sticky top-32">
            {{-- Bagian Gambar: Menangani path 'posters/' dari DB ke folder 'storage/events/' --}}
            @if($event->poster_path)
                @php
                    // Mengambil nama file saja (misal: event-1.png) dari path database (misal: posters/event-1.png)
                    $fileName = basename($event->poster_path);
                @endphp
                <img src="{{ asset('storage/events/' . $fileName) }}" alt="{{ $event->title }}"
